...
import { fetchWalletBalance, fetchWalletTransactions, fetchTokenHoldings, isValidFogoAddress } from "@/lib/fogo-rpc"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [balance, transactions, tokenHoldings] = await Promise.all([
      ...
      fetchWalletTransactions(address), // No limit - fetch all transactions
    ...
    return NextResponse.json({ error: "Failed to process wallet data" }, { status: 500 })
  }
}
